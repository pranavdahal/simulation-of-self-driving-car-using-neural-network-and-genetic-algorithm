...
// generating traffic (x,y,width,height,controlType,maxSpeed)
const traffic=[
    ...
    new Car(road.getLaneCenter(3),-1100,30,50,"DUMMY",2,getRandomColor()),
...
function animate(time){
    ...
    // car with minimum y ==> car that goes the farthest.
    bestCar=cars.find(
        c=>c.y==Math.min(
            ...cars.map(c=>c.y)
        ));
    ...
    carCtx.save();
    // translate to y of car to make the road moving, 0.7 to place the car at bottom 70% of screen
    ...
    // other generated cars dimmed
    carCtx.globalAlpha=0.1;
    for(let i=0;i<cars.length;i++){
        cars[i].draw(carCtx);
        // cars[i].draw(carCtx,"#181407");
    }
    carCtx.globalAlpha=1;
    bestCar.draw(carCtx,true);      //draw car with sensor
